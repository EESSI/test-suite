...
## Modifying Author
Satish Santhosh Kamath, SURF, Netherlands. The license mentioned under is the same as the ones that the original authors
intended and is mentioned below.
...
# Microbenchmark
* Known to run with OpenFOAM-v2106 and OpenFOAM-13 compiled with double precision (WM_PRECISION_OPTION=DP).
...
* With some changes, the benchmark was adopted to be run on [OpenFOAM-13](https://openfoam.org/).
* For the runs with ReFrame, *fixedTol* version is being used.
...
* NOTE: The ReFrame runs do not use the pre-built binaries such as `Allrun`, instead the steps are run separately in the
  ReFrame run script.
* The instructions below apply only if the benchmark is to be run in a standalone manner whereas the parameters are
  fixed within the ReFrame test for runs at different scales and mesh sizes.
